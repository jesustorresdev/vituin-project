...
"""Extracción de viviendas en Holidaylettings
"""
...
from extraction_data.urls import HolidaylettingsURLs
from extraction_data.items import ListHolidaylettingsHomeItem
from extraction_data.required_fields import ListHolidaylettingsHomeRequiredFields
...
ELASTICSEARCH_INDEX = 'holidaylettings_homes'
ELASTICSEARCH_DOC_TYPE = 'unstructured'


#TODO use loaders
class HolidaylettingsSpider(ScrapySpider):
    name = "holidaylettings_listHomes"
    def __init__(self, place=''):
        self.place = place
        self.start_urls = HolidaylettingsURLs(place)
    ...
    def parse_home(self, response):
        ...
        if not item['lat']:
            ...
            (item['lat'],item['lng']) = selenium.get_coordinates_google_map(pos_above="//section[@class ='map-content']", english_page=True)
            selenium.close_and_kill_browser()

        self.check_item(item, ListHolidaylettingsHomeRequiredFields())
