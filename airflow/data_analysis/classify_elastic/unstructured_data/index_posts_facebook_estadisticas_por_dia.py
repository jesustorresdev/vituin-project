...
ELASTICSEARCH_INDEX_SEARCHED='index_posts_facebook_selenium'
ELASTICSEARCH_INDEX='index_posts_stats_by_date'
...
for field in FIELDS:
    ...
    for entry in res:
        ...
        if date not in dates:
            dates.append(date)
            for filter_field in ["comments", "shares", "reproductions", "likes", "loves", \
                ...
                if field == FIELDS[0]:
                    data = utils.get_sum_field(res, filter_field, constraints = constraints, name_options = set_options, date = 10)
                else:
                    sum = utils.get_sum_field(res, filter_field, constraints = constraints, name_options = set_options, date = 10)
                    count = utils.get_count(res, filter_field, constraints = constraints, name_options = set_options, date = 10)
