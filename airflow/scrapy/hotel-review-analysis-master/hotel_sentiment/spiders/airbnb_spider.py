...
class BookingSpider(scrapy.Spider):
    ...
    start_urls = [
        ...
        "https://www.airbnb.es/s/Puerto-de-la-Cruz--Santa-Cruz-de-Tenerife/homes?refinement_paths%5B%5D=%2Fhomes&place_id=ChIJBxuTnlXVQQwRQWOmKGEY6s0&query=Puerto%20de%20la%20Cruz%2C%20Santa%20Cruz%20de%20Tenerife&allow_override%5B%5D=&s_tag=BHax4sSu"
    ...
    def parse(self, response):
        #reviewsurl = response.xpath('//a[@class="hotel_name_link url"]')
        # print 'aqui--------->               ',response.xpath('//a[@class="hotel_name_link url"]/@href')
        # for element in response.xpath('//a[@class="hotel_name_link url"]/@href'):
        #     print element.extract()

        # print 'aqui--------->               ',response.xpath('//main/div')
        print 'aqui--------->               ',response.xpath('//a[@target="listing_6354320"]').extract()
        # for element in response.xpath('//a[@class="hotel_name_link url"]/@href'):
        #     print element.extract()
            #url = response.urljoin(element.extract())
            # yield scrapy.Request(url, callback=self.parse_element)
